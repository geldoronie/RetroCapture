...
public:
    ...
    bool isOpen() const { return m_fd >= 0 || m_dummyMode; }

    bool setFormat(uint32_t width, uint32_t height, uint32_t pixelFormat = 0);
    bool setFramerate(uint32_t fps);

    // Ativar modo dummy (gera frames pretos sem dispositivo real)
    void enableDummyMode(bool enable) { m_dummyMode = enable; }
    bool isDummyMode() const { return m_dummyMode; }
...
private:
    ...
    bool m_dummyMode = false; // Modo dummy: gera frames pretos sem dispositivo

    // Buffer para frame dummy (preto)
    ...
    void generateDummyFrame(Frame &frame); // Gerar frame preto dummy
};
