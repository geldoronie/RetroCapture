    ...
    @autoreleasepool {
        ...
        if (m_captureDevice)
        {
            ...
            if ([m_captureDevice lockForConfiguration:&error])
            {
                ...
                // IMPORTANT: After changing device format, restart the session to apply changes
                // The AVCaptureVideoDataOutput will use the new activeFormat when session restarts
                bool wasRunning = [m_captureSession isRunning];
                if (wasRunning)
                {
                    [m_captureSession stopRunning];
                    // Small delay to ensure format change is applied
                    usleep(100000); // 100ms
                    [m_captureSession startRunning];
                    LOG_INFO("Session restarted to apply new format");
                }
            }
            else
            {
