...
bool MediaEncoder::initializeVideoCodec()
{
    ...
    codecCtx->max_b_frames = 0; // Sem B-frames para reduzir latência e evitar problemas de corrupção
    codecCtx->pix_fmt = AV_PIX_FMT_YUV420P;
    codecCtx->bit_rate = m_videoConfig.bitrate;
    // CRÍTICO: Usar thread_count = 0 (auto) mas desabilitar threading de frame para evitar corrupção
    // Threading de frame pode causar problemas de sincronização e corrupção parcial
    codecCtx->thread_count = 0;              // Auto-detect
    codecCtx->thread_type = FF_THREAD_SLICE; // Apenas threading de slice, não de frame

    // Para codecs que precisam de global header
    // IMPORTANTE: Para H.264 em MPEG-TS, não usar GLOBAL_HEADER se repeat-headers estiver ativo
    // O repeat-headers=1 já garante que SPS/PPS estão nos pacotes, então não precisamos do global header
    if (codec->id == AV_CODEC_ID_HEVC ||
        codec->id == AV_CODEC_ID_VP8 || codec->id == AV_CODEC_ID_VP9)
    {
        codecCtx->flags |= AV_CODEC_FLAG_GLOBAL_HEADER;
    }
    // Para H.264, não usar GLOBAL_HEADER quando repeat-headers=1 está ativo
    // Isso garante que SPS/PPS estejam nos pacotes, não apenas no extradata
    ...
    if (codec->id == AV_CODEC_ID_H264)
    {
        ...
        // CRÍTICO: repeat-headers=1 faz o x264 enviar SPS/PPS em cada keyframe
        // Isso é essencial para MPEG-TS, onde cada keyframe precisa ser decodificável independentemente
        // Sem isso, o decoder gera erros "non-existing PPS referenced" e "Invalid NAL unit"
    ...
    // CRÍTICO: Usar as dimensões configuradas, mas o frame será preenchido com dados convertidos
    // Se os frames de entrada tiverem dimensões diferentes, o sws_scale fará o redimensionamento
...
bool MediaEncoder::convertRGBToYUV(const uint8_t *rgbData, uint32_t width, uint32_t height, void *videoFrame)
{
    ...
    // CRÍTICO: Usar as dimensões configuradas no codec como destino
    // O codec foi inicializado com dimensões específicas e precisa receber frames nessas dimensões
    // Se os frames de entrada tiverem dimensões diferentes, precisamos redimensionar
    ...
    // CRÍTICO: Se as dimensões de entrada diferem das configuradas, precisamos ajustar
    // O codec foi inicializado com dimensões específicas, mas os frames podem ter dimensões diferentes
    // A melhor solução é redimensionar os frames para as dimensões do codec usando sws_scale
    // Isso garante que o codec sempre recebe frames nas dimensões corretas
    if (width != dstWidth || height != dstHeight)
    {
        static int logCount = 0;
        if (logCount < 3)
        {
            LOG_WARN("convertRGBToYUV: Frame dimensions (" + std::to_string(width) + "x" + std::to_string(height) +
                     ") differ from codec config (" + std::to_string(m_videoConfig.width) + "x" + std::to_string(m_videoConfig.height) +
                     "). Resizing to codec dimensions.");
            logCount++;
        }
        // Manter dstWidth e dstHeight como m_videoConfig para redimensionar corretamente
        // O sws_scale fará o redimensionamento de width x height para dstWidth x dstHeight
    }

    // Obter ou criar SwsContext
    ...
    // CRÍTICO: Validar que temos dados suficientes antes de converter
    // O tamanho esperado é width * height * 3 bytes para RGB24
    size_t expectedSize = static_cast<size_t>(width) * static_cast<size_t>(height) * 3;
    // Nota: Não podemos validar o tamanho real do rgbData aqui pois é apenas um ponteiro
    // Mas podemos validar que os parâmetros são consistentes
    ...
    // CRÍTICO: Verificar que sws_scale processou todas as linhas de SAÍDA
    // O sws_scale retorna o número de linhas de SAÍDA processadas, não de entrada
    // Quando redimensionamos (ex: 1056 -> 1080), é normal que ret > height (entrada)
    // Portanto, devemos comparar com dstHeight (saída esperada)
    if (ret != static_cast<int>(dstHeight))
    {
        LOG_ERROR("sws_scale processed " + std::to_string(ret) + " output lines, expected " + std::to_string(dstHeight) +
                  " (input: " + std::to_string(width) + "x" + std::to_string(height) +
                  ", output: " + std::to_string(dstWidth) + "x" + std::to_string(dstHeight) + ")");
        return false;
    }

    // Nota: Não é um problema se ret > height quando estamos redimensionando
    // O sws_scale está fazendo seu trabalho corretamente ao redimensionar
                               ...
                               int64_t captureTimestampUs, std::vector<EncodedPacket> &packets)
{
    ...
    // CRÍTICO: Validar tamanho dos dados RGB antes de processar
    // RGB24 = 3 bytes por pixel
    size_t expectedSize = static_cast<size_t>(width) * static_cast<size_t>(height) * 3;
    ...
    {
        LOG_ERROR("MediaEncoder: convertRGBToYUV failed (width=" + std::to_string(width) +
    ...
    if (forceKeyframe)
    {
        ...
        // CRÍTICO: Forçar keyframe também via codec context
        // Isso garante que o encoder realmente gere um keyframe com SPS/PPS
        AVCodecContext *ctx = static_cast<AVCodecContext *>(m_videoCodecContext);
        if (ctx && (ctx->codec_id == AV_CODEC_ID_H264 || ctx->codec_id == AV_CODEC_ID_HEVC))
        {
            // Forçar IDR frame (keyframe) para H.264/H.265
            videoFrame->pict_type = AV_PICTURE_TYPE_I;
        }
    }
    m_videoFrameCount++;
    ...
    if (ret < 0)
    {
        ...
        if (ret < 0 && ret != AVERROR(EAGAIN))
        {
            LOG_ERROR("MediaEncoder: avcodec_send_frame failed: " + std::to_string(ret));
            return false;
        }
    }
...
bool MediaEncoder::receiveVideoPackets(std::vector<EncodedPacket> &packets, int64_t captureTimestampUs)
{
    ...
    while (true)
    {
        ...
        // CRÍTICO: Validar pacote antes de copiar
        if (!pkt->data || pkt->size <= 0 || pkt->size > 10 * 1024 * 1024) // Máximo 10MB por pacote
        {
            LOG_WARN("MediaEncoder: Invalid video packet (data=" + std::to_string(pkt->data != nullptr) +
                     ", size=" + std::to_string(pkt->size) + "), skipping");
            av_packet_unref(pkt);
            continue;
        }

        EncodedPacket encodedPkt;
        // CRÍTICO: Copiar dados ANTES de unref para evitar corrupção
        // Validar e copiar dados de forma segura
        encodedPkt.data.reserve(pkt->size); // Reservar espaço para evitar realocações
        encodedPkt.data.assign(pkt->data, pkt->data + pkt->size);

        // Validar que a cópia foi bem-sucedida
        if (encodedPkt.data.size() != static_cast<size_t>(pkt->size))
        {
            LOG_ERROR("MediaEncoder: Failed to copy video packet data (expected " + std::to_string(pkt->size) +
                      " bytes, got " + std::to_string(encodedPkt.data.size()) + ")");
            av_packet_unref(pkt);
            continue;
        }

        // Validar primeiros bytes para detectar corrupção básica
        if (encodedPkt.data.size() > 0 && encodedPkt.data[0] != pkt->data[0])
        {
            LOG_ERROR("MediaEncoder: Data corruption detected in video packet copy (first byte mismatch)");
            av_packet_unref(pkt);
            continue;
        }
...
bool MediaEncoder::receiveAudioPackets(std::vector<EncodedPacket> &packets, int64_t captureTimestampUs)
{
    ...
    while (true)
    {
        ...
        if (!pkt->data || pkt->size <= 0 || pkt->size > 10 * 1024 * 1024) // Máximo 10MB por pacote
        {
            LOG_WARN("MediaEncoder: Invalid audio packet (data=" + std::to_string(pkt->data != nullptr) +
                     ", size=" + std::to_string(pkt->size) + "), skipping");
            av_packet_unref(pkt);
            continue;
        }
        ...
        if (encodedPkt.data.size() != static_cast<size_t>(pkt->size))
        {
            LOG_ERROR("MediaEncoder: Failed to copy audio packet data (expected " + std::to_string(pkt->size) +
                      " bytes, got " + std::to_string(encodedPkt.data.size()) + ")");
            av_packet_unref(pkt);
            continue;
        }
        ...
        if (encodedPkt.data.size() > 0 && encodedPkt.data[0] != pkt->data[0])
        {
            LOG_ERROR("MediaEncoder: Data corruption detected in audio packet copy (first byte mismatch)");
            av_packet_unref(pkt);
            continue;
        }
...
void MediaEncoder::flush(std::vector<EncodedPacket> &packets)
{
    ...
    if (videoCtx)
    {
        avcodec_send_frame(videoCtx, nullptr); // Flush
    ...
    if (audioCtx)
    {
        avcodec_send_frame(audioCtx, nullptr); // Flush
